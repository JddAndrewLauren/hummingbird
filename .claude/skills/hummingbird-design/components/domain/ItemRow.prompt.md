...
<ItemRow title="Order the replacement sensor" stage="ready" urgency="soon"
  due="Fri" size="quick" steps="2/5" onClick={open} />
```
...
The urgency dot's tooltip reads in words ("Due soon"), not as the stored enum. `onMouseEnter`, `onMouseLeave` and `onKeyDown` passed in are called after the row's own, not instead of them.
