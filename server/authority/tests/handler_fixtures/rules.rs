...
/// #137's own two mapping entries, exercised the same way as #135's: an
/// `ingest` token bound to `m365-mail/v1` reads the `email`-kind rule (its
/// own kind) and the any-kind rule, but never the `calendar_event`-kind
