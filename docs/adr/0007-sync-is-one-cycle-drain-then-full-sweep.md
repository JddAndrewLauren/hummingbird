...
all stand.
**Completed 2026-08-09 by [ADR-0010](0010-one-core-per-origin.md):** this ADR
specifies the cycle but never says how many run against one origin. Exactly one —
the core lives in a `SharedWorker`, and a second tab or the installed PWA window is
a view, not a second cycle.
