...
| `frontier-drag.ts`'s drop mapping (sunk at #801) | the rule sank (`decisions::frontier`'s `drop_edits` — which field a card dropped into a column writes, ADR-0021 decision 9). It is `group_frontier`'s inverse and reads the same axis vocabulary, the same blank-fold and the same urgency arithmetic, and its same-column test **is** `group_frontier`'s own `axis_value`; a round-trip gate re-groups an edited item so the two can never disagree. `client/ffi-web`'s `frontier_drop_edits_json` emits `protocol.ts`'s `TriageEdits` verbatim, pinned by a round-trip through the worker's own inbound deserializer |
| The board drag's mobile seam (#801) | two doors over the same sunk rule, not one, because the phone cannot ask it per frame: `droppable_columns` answers which columns a card may land in (once, when a press becomes a drag, so a refused column is never lit) and `move_item_to_column` commits (once, when the finger lifts). Both call `drop_edits`; Kotlin sees a column key and a boolean, and a structural test refuses any band, field or default-context word in the board's own source |
| `frontier-drag.ts`'s physics, hit-testing and edge auto-scroll (#801) | stays per client, on `frontier-lanes.ts`'s own verdict: it consumes measured boxes. The spring, the frozen column rects, the hold-to-arm rule and the scroll correction are all statements about one host's layout and one host's input model, and Android's `NowLaneBoard` port expresses the same physics in Compose's own `Animatable`/`spring` rather than sharing an integrator |
