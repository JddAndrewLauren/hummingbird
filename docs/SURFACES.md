...
| **Components** | `components/{core,forms,domain,feedback}/` — the 19-component library. Counted as *components a screen calls*: the eight size/energy glyph primitives in `core/custom-glyphs.tsx` (#446, ADR-0024) are not among them, since they exist only to fill `Icon`'s `ICON_MAP` and no caller names one directly. |
