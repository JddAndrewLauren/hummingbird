...
Four widths × two themes × twenty-two screen states, per run.
...
projects-dossier batch's four dossier states, and back to twenty-two when
...
`-no-actions-*` became `-empty-board-*`, and `-slot-open-*` arrived). Not by widening the kit world to mount the board, which the
