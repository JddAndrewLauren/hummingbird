...
> **Status: proven end to end, 2026-08-23 (#712), against a local
> `wrangler dev` + `pnpm dev` stack — not against the deployed
> `hb.twinion.net`.** This slice's own commits (#706–#711) are on the
> `diagnostics-slices` integration branch, not yet merged to `main`, so the
> Worker actually serving `hb.twinion.net` today predates every event this
> runbook talks about — a live `wrangler tail` against it returns
> `"logs": []` for every request, not because nothing was logged but because
> the deployed build cannot log it yet. Every gesture below is the exact
> gesture an operator runs against production once this lane ships; the
> **Proof** section names, per row, whether it was run against production
> or against a local `wrangler dev`/`pnpm dev` pair standing in for it, and
> why.
...
Every row, on every host, is the same envelope:

```json
{"schema_version":1,"seq":42,"wall_clock_ms":1787500000000,"elapsed_ms":12345,
 "session_id":"...","source":"web-worker","cycle_id":"c-7","operation_id":"op-3",
 "request_id":null,"event":{"name":"http.started","payload":{...}}}
```
...
Object) logs one `request.received` and one `request.finished` line per
request it authenticates, both carrying the client-minted
...
|---|---|
| Client `http.started`, no server `request.received` | Failure before the authority |
| Server `request.received`, no server `request.finished` | Authority/DO stall |
| Server `request.finished`, no client `http.finished` | Response path or client transport |
| Sync owns core while UI operation waits (`core.busy`/`core.wait_started` naming the sync session, an interactive `operation.requested` stuck behind it) | Core monopolization confirmed |
| `request.finished{status:401}` followed by the client holding its credential invalid (no further `http.started` until the token changes) | Credential rejection |
| `operation.local_commit` with no later `http.started` for that write | Sync/transport failure |
| `operation.local_commit` with no later UI-visible state change | Client state/rendering failure |
| No Android `worker.started` in the window the push/schedule should have fired | OS scheduling or push-delivery issue |
...
Every row below was reproduced against the real code in this worktree, not
a fixture — a local `wrangler dev` (the real `hummingbird-authority` Worker,
compiled to wasm, over real HTTP on `127.0.0.1:8787`) and, for the client
rows, `pnpm dev`'s real Vite dev server serving the real PWA (proxying
`/api` to that same `wrangler dev`, `client/web/vite.config.ts`), driven by
a real Chromium instance. This stands in for `hb.twinion.net` for the reason
in the banner above: production has not deployed this batch yet. Every
artefact cited here was redaction-checked (see **Redaction** below) before
being cited.
...
the device's network stack, exactly row 1's pattern.
...
inside the Durable Object's `fetch`, not a fast round trip misread.
...
- **Authority Workers Logs: 3 days on the Free plan, 7 days on Paid** — a
  ...
  (`server/worker/wrangler.toml`'s `[observability]` comment).
