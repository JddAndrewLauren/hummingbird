...
`sweep.py` is the one-way capture → Linear Triage sweeper — one drain engine
...
invariants (one frozen namespace per source, Linear-first ordering, no Actions
`schedule:`, no `[http_service]`) are load-bearing and decided upstream; read
`docs/sweeper.md` before touching any of them.
