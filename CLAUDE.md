...
| The Projects page and the project lane's client write (#624, the first slice off #449) — grid, create, dossier shell, the properties card (#625: `github_repo`/`default_context`) and the links card (#626: `project_links`, ADR-0030 decision 4) | `client/core/src/lib.rs`'s `create_project`/`patch_project`/`create_project_link`/`patch_project_link`, `client/ffi-web/src/{task_host,lib}.rs`, `client/web/src/{shell/useProjectsWiring.ts,screens/ProjectsScreen.tsx,screens/projects/}` | `ProjectsScreen.tsx`, then `Core::create_project`'s doc for the no-overlay contract; ADR-0030 |
