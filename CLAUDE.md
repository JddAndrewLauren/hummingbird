...
| The watch (ADR-0039) — a device with its own core and its own `device-watch` token: the `:wear` module over the two libraries the phone shares with it (`:core-binding`, the Rust seam and host core package; `:brand`, tokens, type, glyphs and the pane words), the phone-delivered token, capture through the system input chooser, the Now questions in the core's order, one capture tile | `client/android/{core-binding,brand,wear}/`, `client/android/app/src/main/kotlin/net/twinion/hummingbird/watch/` | `wear/build.gradle.kts`'s header, then `android/README.md`'s "The watch" section; ADR-0039 |
...
| An item points at one note in the Obsidian vault (#771) — the `items.vault_path` column, the `obsidian-vault` binding, and the one place all the Obsidian vendor knowledge lives (the `&append` flag is load-bearing; its own header says why), plus the item panel's read-mode affordance over it — add (the path is proposed and editable before it is stored), open, edit-or-unlink. Since the attachment row (below) it is one of three, and its resting label is `Add` plus the note glyph. Web only: the phone carries the column and draws nothing | `server/domain/src/item.rs`, `server/authority/src/{schema.rs,handlers/items.rs}`, `client/core/src/bindings.rs`, `client/web/src/obsidian/vault-uri.ts`, `client/web/src/components/domain/{NoteLink,ItemPanel}.tsx` | `client/web/src/obsidian/vault-uri.ts`, then ADR-0009's `items.vault_path` amendment; ADR-0002/0015 as amended |
| An item carries one **Link** and Android is a share target (#782) — the `items.link_url`/`items.link_label` pair on both clients, the share-payload mapping decided once in the core, the `.ShareTarget` alias over `CaptureActivity`, and the link affordance wherever an item is opened — `LinkAttach`, one of the three in the attachment row below, which replaced the anchor-plus-`Edit link` pair | `client/core/src/decisions/share.rs`, `server/domain/src/item.rs`, `server/authority/src/{schema.rs,handlers/items.rs}`, `client/ffi-mobile/src/lib.rs`'s `parse_share_payload`/`link_display_label`, `client/ffi-web/src/decisions.rs`, `client/web/src/{screens/CaptureBox.tsx,components/domain/{LinkAttach,ItemPanel}.tsx}`, `client/android/.../{CaptureActivity.kt,ui/forms/LinkField.kt,ItemDetailPanel.kt}`, `client/android/app/src/main/AndroidManifest.xml` | `decisions/share.rs`, then `client/android/README.md`'s "The share target" section; ADR-0009's Link amendment, ADR-0025 |
| An item points at many files in Dropbox (ADR-0036) — the `file_links` table (add and remove only, never re-pointed), the `hummingbird-open:` scheme the web fires and the once-per-machine helper that claims it (the machine-local root lives there, not in a binding), the always-drawn dropbox.com fallback, and the one place all the Dropbox vendor knowledge lives. Web only: the phone carries the table and draws nothing | `server/domain/src/file_link.rs`, `server/authority/src/{schema.rs,handlers/file_links.rs}`, `client/core/src/lib.rs`'s `file_links_for`/`create_file_link`/`remove_file_link`, `client/web/src/dropbox/`, `client/web/src/shell/useFileLinksWiring.ts`, `client/web/src/components/domain/{FileAttach,ItemPanel}.tsx`, `tools/dropbox-open/` | `client/web/src/dropbox/file-link.ts`, then `tools/dropbox-open/README.md`; ADR-0036 |
| The browser capture extension (ADR-0037) — a popup-only Manifest V3 extension that captures the active tab as a new item, the URL as its Link, `POST`ed straight to the authority from the popup page under its own device token (`device-chrome-<machine>`); no server change, no CORS, the share mapping's title rules re-stated once in JS and pinned by test, the web's design tokens copied under a drift gate. Chrome today; Firefox and Safari are ports of the same tree | `tools/browser-capture/`, `.github/workflows/browser-capture.yml` | `tools/browser-capture/lib.js`'s header, then its `README.md`; ADR-0037 |
| The three things an item points at are one row, on both surfaces — **link** (#782), **note** (#771) and **file** (ADR-0036), each reading `Add` and its glyph, each opening an editor in place. Behind capture's "More details" chevron; in the item panel's read mode, beside each other and no longer sharing a row with Grill me. The note and the file **cannot ride on the capture** (`Core::capture` hardcodes `vault_path: None`, and a file link needs an item id the authority checks), so the box collects them and a follow-up write attaches them once the capture's result names the minted id. Web only | `client/web/src/screens/CaptureBox.tsx`'s `AttachToggle`, `client/web/src/shell/useCaptureAttachments.ts`, `client/web/src/components/domain/{LinkAttach,NoteLink,FileAttach}.tsx`, `client/web/src/components/core/custom-glyphs.tsx`'s `DropboxMark` | `useCaptureAttachments.ts`, then each component's own header |
...
| The suggested contexts are an edited list (ADR-0038) — the third `settings` vocabulary (one ordered `contexts` row, defaults until edited), the two Core doors and the removal cascade (list first, then one clear per live item and per project default, archived never), both seams, Settings' `Contexts` section and the live list reaching capture, the item panel and the frontier chips. Web edits it; the phone reads it through an injected door beside `captureFormMeta` | `client/core/src/contexts.rs`, `client/core/src/lib.rs`'s `suggested_contexts`/`add_context`/`remove_context`, `client/ffi-web/src/task_host.rs`, `client/ffi-mobile/src/lib.rs`'s `suggested_contexts`, `client/web/src/screens/{contexts.ts,SettingsScreen.tsx,field-vocabulary.ts}`, `client/android/.../{CaptureViewModel,ItemDetailViewModel}.kt` | `contexts.rs`, then ADR-0038 |
