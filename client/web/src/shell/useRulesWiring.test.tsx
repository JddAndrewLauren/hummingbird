...
import { useRulesWiring, type RulesWiring } from "./useRulesWiring";
...
let latest: RulesWiring | null = null;
...
function Probe({
  ...
  latest = useRulesWiring(worker, status, syncOutcomeSeq);
  return null;
}
...
describe("useRulesWiring", () => {
  ...
  it("createRule posts a createRule message with a minted seed", () => {
    ...
    latest!.createRule("A new rule", "capture", [], "warn", "normal", true);
  ...
  it("patchRule posts a patchRule message carrying the current row and a deterministic seed", () => {
    ...
    latest!.patchRule(rule, { enabled: false });
  ...
  it("patchRule marks deletedAt as touched when the patch sets it", () => {
    ...
    latest!.patchRule(rule, { deletedAt: 5_000 });
