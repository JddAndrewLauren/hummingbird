...
impl<'a> DiagnosticSession<'a> {
    ...
    /// second copy of `seq`/envelope bookkeeping. `wall_clock_ms` and
    /// `elapsed_ms` are both caller-supplied, same as everywhere else in
    /// this module — this method samples no clock of its own.
    pub fn emit(
        &self,
